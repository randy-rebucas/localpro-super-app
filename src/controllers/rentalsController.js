...
// @desc    Generate rental description from name using AI
...
const generateRentalDescription = async (req, res) => {
  try {
    ...
    if (options) {
      ...
      if (options.category && ['tools', 'vehicles', 'equipment', 'machinery'].includes(options.category)) {
    ...
    res.status(200).json({
      ...
      data: {
        ...
        tags: result.tags,
        wordCount: result.wordCount || result.description.trim().split(/\s+/).filter(word => word.length > 0).length,
        usage: aiResponse.usage,
        debug: process.env.NODE_ENV === 'development' ? aiResponse.debug : undefined
      }
    });
  } catch (error) {
    logger.error('Generate rental description error', {
      error: error.message,
      stack: error.stack,
      userId: req.user?.id
    });
    ...
    res.status(500).json({
      success: false,
      message: 'Failed to generate rental description',
      ...(process.env.NODE_ENV === 'development' && { error: error.message })
    });
  }
};
