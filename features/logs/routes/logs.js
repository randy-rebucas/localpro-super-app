...
router.get('/stats', auth, async (req, res) => {
  try {
    const userRoles = req.user.roles || [];
    const isAdminUser = req.user.hasRole ? req.user.hasRole('admin') : userRoles.includes('admin');
    if (!isAdminUser) {
      return res.status(403).json({
        success: false,
        message: 'Access denied. Admin privileges required.'
...
router.get('/search/global', auth, async (req, res) => {
  try {
    ...
    const { q: queryStr, timeframe = '7d' } = req.query;
    ...
    const results = await logManagementService.searchLogs(queryStr, timeframe);
